...
In general, datasets must follow the [FAIR](https://www.go-fair.org/fair-principles/) principles:
...
::: callout-tip 
Visit this [resource](https://github.com/Alliance-RDM-GDR/RDM_OrganizeData) for principles on handling and organizing research data.
:::
...
:::: {#second-column}
::: {style="text-align: center;font-size: 100%"}
![This presentation is available [here](https://github.com/Alliance-RDM-GDR/RDM_DepositingData) (English or French)](images/Presentation_QR-code.png){fig-alt="A QR code image that redirects to the presentation located in a GitHub repository." fig-align="center" width="200" height="200"}
:::
::::
:::::
...
document.addEventListener("DOMContentLoaded", () => {
  const slides = document.querySelectorAll(".slide[data-caption-src]");

  // The global audio player added by the plugin
  const globalAudio = document.querySelector(".audio-controls audio");

  slides.forEach(slide => {
    const captionUrl = slide.getAttribute("data-caption-src");
    const captionBox = slide.querySelector("#custom-caption-box");

    if (!captionUrl || !captionBox || !globalAudio) return;
    ...
    fetch(captionUrl)
      ...
      .then(vtt => {
        ...
        for (let i = 0; i < lines.length; i++) {
          if (lines[i].includes("-->")) {
            const [start, end] = lines[i].split(" --> ").map(t => toSeconds(t));
        ...
        globalAudio.addEventListener("timeupdate", () => {
          if (!slide.classList.contains("present")) return; // only update for active slide
